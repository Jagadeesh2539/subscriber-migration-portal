...
def validate_environment():
    ...
    for var, description in required_vars.items():
        ...
        if not value:
            ...
            raise ValueError(f"JWT_SECRET must be at least 32 characters long for security")
...
def load_users_from_secrets() -> Dict:
    ...
    try:
        ...
        logger.info(f"Loaded {len(users)} users from Secrets Manager")
        return users
    
    except Exception as e:
        logger.error(f"Failed to load users from Secrets Manager: {str(e)}")
...
class SecureUserManager:
    ...
    def record_failed_attempt(username: str):
        ...
        if len(login_attempts[username]) >= CONFIG['MAX_LOGIN_ATTEMPTS']:
            ...
            logger.warning(f"Account locked due to failed attempts: {InputValidator.sanitize_string(username, 50)}")
    ...
    try:
        ...
        if CONFIG.get('LEGACY_DB_SECRET_ARN'):
            try:
                ...
                logger.error(f"Legacy DB stats error: {str(e)}")
...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info(f"Lambda invoked - Method: {event.get('httpMethod', 'UNKNOWN')}, Path: {event.get('path', 'UNKNOWN')}")
        ...
        logger.error(f"Lambda handler error [{error_id}]: {str(e)}")
