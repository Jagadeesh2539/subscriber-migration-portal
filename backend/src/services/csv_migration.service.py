...
class CSVMigrationService:
    ...
    def process_csv_migration(self, csv_file, migration_id: str, 
                             cloud_migration_enabled: bool = True) -> MigrationSummary:
        """
        ...
        try:
            ...
            self.audit_service.log_action(
                ...
                details={
                    ...
                    'cloud_migration_enabled': cloud_migration_enabled,
                    'total_processed': stats['total_processed'],
                    'success_rate': (stats['migrated_successfully'] / stats['total_processed'] * 100) if stats['total_processed'] > 0 else 0
                }
            )
            
            logger.info(f"Migration {migration_id} completed: {stats['migrated_successfully']} successful, {stats['failed']} failed")
    ...
    def _process_single_subscriber(self, identifier: str, identifier_type: str, 
                                 migration_id: str, cloud_migration_enabled: bool,
                                 row_num: int) -> MigrationRecord:
        """
        ...
        try:
            ...
            if not legacy_subscriber:
                ...
                self._log_migration_event(migration_id, identifier, 'NOT_FOUND', 
                                        f'Subscriber not found in legacy system', row_num)
            ...
            if cloud_subscriber:
                ...
                self._log_migration_event(migration_id, identifier, 'ALREADY_PRESENT', 
                                        'Already present in cloud', row_num)
                return MigrationRecord(
                    identifier=identifier,
                    identifier_type=identifier_type,
            ...
            if not cloud_migration_enabled:
                self._log_migration_event(migration_id, identifier, 'SIMULATION', 
                                        'Cloud migration disabled for this Migration ID', row_num)
                return MigrationRecord(
                    identifier=identifier,
                    identifier_type=identifier_type,
                    status='SIMULATION',
            ...
            if migration_result['success']:
                self._log_migration_event(migration_id, identifier, 'SUCCESS', 
                                        'Migrated successfully to cloud', row_num)
                return MigrationRecord(
                    identifier=identifier,
                    identifier_type=identifier_type,
                    status='SUCCESS',
                ...
                # Migration failed
                self._log_migration_event(migration_id, identifier, 'FAILED', 
                                        f'Migration failed: {migration_result["error"]}', row_num)
                return MigrationRecord(
                    identifier=identifier,
                    identifier_type=identifier_type,
                    status='FAILED',
    ...
    def _log_migration_event(self, migration_id: str, identifier: str, status: str, 
                           reason: str, row_num: int):
    ...
    def _store_migration_summary(self, summary: MigrationSummary):
        ...
        try:
            ...
            item = {
                ...
                'success_rate': Decimal(str(round(
                    (summary.migrated_successfully / summary.total_processed * 100) 
                    if summary.total_processed > 0 else 0, 2
                ))),
